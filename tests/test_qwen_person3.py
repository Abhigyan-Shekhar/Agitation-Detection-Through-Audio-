...
def test_qwen_call_uses_qwen_json_object_mode_and_strict_prompt(record):
    ...
    assert completions.kwargs[0]["reasoning_format"] == "hidden"
...
def test_think_block_followed_by_json(record):
    content = f"<think>Here is a thinking process with {{not: 'json'}} inside.</think>\n{valid_response()}"

    result = validate_qwen_response(content, record)
...
def test_think_block_plus_markdown_fenced_json(record):
    content = f"<think>reasoning that should not reach dashboard</think>\n```json\n{valid_response()}\n```"

    result = validate_qwen_response(content, record)

    assert result.validated is True
    assert result.explanation == "The repeated question is supported by timestamped transcript segments."


def test_only_think_block_has_clear_error(record):
    with pytest.raises(QwenResponseValidationError, match="only reasoning"):
        validate_qwen_response("<think>no final answer</think>", record)
